...
int compare_int_reverse(const void *, const void *);
...
void HyperCube_Class::check_list()
{
	...
	// Check that the local list is sorted and that elements are smaller than
	// or equal to the smallest on process with rank (my_id-1)
...
void HyperCube_Class::HyperCube_QuickSort()
{
	...
	// Sort local list
	qsort(list, list_size, sizeof(int), compare_int_reverse);
	...
	for (k = dimension; k > 0; k--)
	{
		...
		if (nbr_k > my_id)
		{
			// MPI-2: Send number of elements smaller than pivot

			// ***** Add MPI call here *****
			MPI_Send(&list_size_lt, 1, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
			...
			// ***** Add MPI call here *****
			MPI_Send((list + idx), list_size_lt, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
		...
		else
		{
			...
			// ***** Add MPI call here *****
			MPI_Send(&list_size_geq, 1, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
			...
			// ***** Add MPI call here *****
			MPI_Send(list, list_size_geq, MPI_INT, nbr_k, 0, MPI_COMM_WORLD);
...
int compare_int_reverse(const void *a0, const void *b0)
{
	int a = *(int *)a0;
	int b = *(int *)b0;
	if (a > b)
