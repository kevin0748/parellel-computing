...
// Sort local lists
#pragma omp parallel default(none) shared(list, ptr) firstprivate(num_threads) private(my_list_size)
    {
#pragma omp for
        for (my_id = 0; my_id < num_threads; my_id++)
        {
...
#pragma omp parallel default(none) shared(list, ptr, work) firstprivate(num_threads, np, level, list_size)
        {
#pragma omp for
            for (my_id = 0; my_id < num_threads; my_id++)
            {
