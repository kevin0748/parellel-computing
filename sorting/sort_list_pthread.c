...
int localsort_cnt;
pthread_mutex_t localsort_lock;
pthread_cond_t localsort_cond;

int *work_cnt;
pthread_mutex_t work_lock;
pthread_cond_t work_sync_cond;

int *rearrange_cnt;
pthread_mutex_t rearrange_lock;
pthread_cond_t rearrange_sync_cond;
...
void *thread_merge_sort(void *args)
{
    ...
    pthread_mutex_lock(&localsort_lock);
    localsort_cnt++;
    while (localsort_cnt < num_threads)
    {
        pthread_cond_wait(&localsort_cond, &localsort_lock);
    }
    pthread_cond_signal(&localsort_cond);
    pthread_mutex_unlock(&localsort_lock);

    if (DEBUG)
        printf("my_id: %d sort finished, localsort_cnt: %d \n", my_id, localsort_cnt);
    ...
    for (level = 0; level < q; level++)
    {
        ...
        work_cnt[level]++;
        while (work_cnt[level] < num_threads)
        {
            pthread_cond_wait(&work_sync_cond, &work_lock);
        }
        pthread_cond_signal(&work_sync_cond);
        ...
        pthread_mutex_lock(&rearrange_lock);
        rearrange_cnt[level]++;
        while (rearrange_cnt[level] < num_threads)
        {
            pthread_cond_wait(&rearrange_sync_cond, &rearrange_lock);
        }
        pthread_cond_signal(&rearrange_sync_cond);
        pthread_mutex_unlock(&rearrange_lock);
    }
}

void sort_list_with_thread(int q)
{
    int i, level, my_id;
    int np, my_list_size;
    ...
    int s;
    struct thread_input inputs[num_threads + 1];
    ...
    {
        ptr[my_id] = my_id * np;
        inputs[my_id].my_id = my_id;
        inputs[my_id].ptr = ptr;
        inputs[my_id].q = q;
    }
    ...
    for (my_id = 0; my_id < num_threads; my_id++)
    {
        s = pthread_create(&threads[my_id], NULL, &thread_merge_sort, &inputs[my_id]);
        if (s != 0)
        {
            perror(s);
        }
...
int main(int argc, char *argv[])
{
    ...
    pthread_mutex_init(&localsort_lock, NULL);
    localsort_cnt = 0;
    pthread_cond_init(&localsort_cond, NULL);

    pthread_mutex_init(&work_lock, NULL);
    work_cnt = (int *)calloc(q, sizeof(int));
    pthread_cond_init(&work_sync_cond, NULL);

    pthread_mutex_init(&rearrange_lock, NULL);
    rearrange_cnt = (int *)calloc(q, sizeof(int));
    pthread_cond_init(&rearrange_sync_cond, NULL);
    ...
    free(work_cnt);
    free(rearrange_cnt);
    pthread_mutex_destroy(&localsort_lock);
    pthread_mutex_destroy(&work_lock);
    pthread_mutex_destroy(&rearrange_lock);
    pthread_cond_destroy(&localsort_cond);
    pthread_cond_destroy(&work_sync_cond);
    pthread_cond_destroy(&rearrange_sync_cond);
    free(threads);
}
